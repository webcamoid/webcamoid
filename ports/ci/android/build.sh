...
if [ "${UPLOAD}" == 1 ]; then
    EXTRA_PARAMS="${EXTRA_PARAMS} -DNOGSTREAMER=ON -DNOLIBAVDEVICE=ON -DNOQTCAMERA=ON -DNOQTSCREENCAPTURE=ON -DNOLSMASH=ON -DNOLIBMP4V2=ON -DNOX264=ON -DNOFDKAAC=ON -DNOFAAC=ON -DNOLAME=ON -DNOLIBWEBM=ON -DNOLIBVPX=ON -DNOAOMAV1=ON -DNOSVTAV1=ON -DNORAVIE=ON -DNOLIBOPUS=ON -DNOLIBVORBIS=ON"
fi
