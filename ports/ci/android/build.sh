...
if [ "${UPLOAD}" == 1 ]; then
    EXTRA_PARAMS="${EXTRA_PARAMS} -DNOGSTREAMER=ON -DNOLIBAVDEVICE=ON -DNOQTCAMERA=ON -DNOQTSCREENCAPTURE=ON -DNONDKMEDIA=ON -DNOLSMASH=ON -DNOLIBMP4V2=ON -DNOX264=ON -DNOFDKAAC=ON -DNOFAAC=ON -DNOLAME=ON"
fi
