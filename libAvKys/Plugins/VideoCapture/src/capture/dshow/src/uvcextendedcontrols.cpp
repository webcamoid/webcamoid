...
class UvcExtendedControlsPrivate
{
    public:
        ...
        QString devicePath(IBaseFilter *camera) const;
...
UvcExtendedControls::UvcExtendedControls(IBaseFilter *filter):
    QObject()
{
    this->d = new UvcExtendedControlsPrivate;
    this->d->loadVendors();
...
void UvcExtendedControls::load(IBaseFilter *filter)
{
    this->load(this->d->devicePath(filter));
}

QVariantList UvcExtendedControls::controls(IBaseFilter *filter) const
{
...
bool UvcExtendedControls::setControls(IBaseFilter *filter,
...
QString UvcExtendedControlsPrivate::devicePath(IBaseFilter *camera) const
{
    if (!camera)
        return {};

    // Get the filter's name to match with the moniker

    FILTER_INFO filterInfo;
    auto hr = camera->QueryFilterInfo(&filterInfo);

    if (FAILED(hr))
        return {};

    auto filterName = QString::fromWCharArray(filterInfo.achName);
    ...
    // Search for the moniker matching the filter's name
    ...
    while (enumMoniker->Next(1, &moniker, nullptr) == S_OK) {
        ...
        if (SUCCEEDED(hr) && propBag) {
            ...
            if (SUCCEEDED(hr) && varName.vt == VT_BSTR && varName.bstrVal) {
                auto friendlyName = QString::fromWCharArray(varName.bstrVal);

                if (friendlyName == filterName) {
                    // Found the matching device, read DevicePath
                    VARIANT varPath;
                    VariantInit(&varPath);
                    hr = propBag->Read(L"DevicePath", &varPath, nullptr);

                    if (SUCCEEDED(hr)
                        && varPath.vt == VT_BSTR
                        && varPath.bstrVal) {
                        result = QString::fromWCharArray(varPath.bstrVal);
                    }
                                                ...
                                                quint8 &address) const
{
    ...
    auto path = devicePath.toLower();

    // Parse vendor and product

    QRegularExpression vidRx("vid_([0-9a-f]{4})");
    QRegularExpression pidRx("pid_([0-9a-f]{4})");
    auto vidMatch = vidRx.match(path);
    auto pidMatch = pidRx.match(path);

    if (vidMatch.hasMatch())
        vendorId = vidMatch.captured(1).toUShort(nullptr, 16);

    if (pidMatch.hasMatch())
        productId = pidMatch.captured(1).toUShort(nullptr, 16);
    ...
    for (DWORD index = 0; ; ++index) {
        ...
        if (_wcsicmp(devicePath.toStdWString().c_str(), detailData->DevicePath) != 0) {
            free(detailData);
