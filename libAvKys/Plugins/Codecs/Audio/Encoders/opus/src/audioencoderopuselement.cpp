...
AkPacket AudioEncoderOpusElement::iAudioStream(const AkAudioPacket &packet)
{
    ...
    qsizetype bufferSize =
            sizeof(opus_int16) * src.caps().channels() * src.samples();
...
QByteArray AudioEncoderOpusElementPrivate::readBuffer(int *readSamples)
{
    ...
    auto rate = caps.rate();
    auto totalSamples =
            this->m_audioBuffer.size() / (sizeof(opus_int16) * channels);
    ...
    for (qsizetype i = nDurations - 1; i >= 0; --i) {
        ...
        if (totalSamples >= maxSamples) {
            auto readBytes = sizeof(opus_int16) * channels * maxSamples;
...
bool AudioEncoderOpusElementPrivate::init()
{
    ...
    this->m_outputCaps = {AkCompressedAudioCaps::AudioCodecID_opus,
                          outputCaps.bps(),
                          outputCaps.channels(),
                          outputCaps.rate()};
    this->updateHeaders();
