...
GridLayout {
    ...
    TextField {
        ...
        onTextChanged: VideoMuxerFFmpeg.bitrate = Number(text)
    }
    Label {
        id: txtGOP
    ...
    TextField {
        ...
        onTextChanged: VideoMuxerFFmpeg.gop = Number(text)
    }
    GroupBox {
    ...
    RowLayout {
        ...
        ComboBox {
            ...
            textRole: "text"
            currentIndex: VideoMuxerFFmpeg.deadline == VideoMuxerFFmpeg.Deadline_Realtime?
                                0:
                          VideoMuxerFFmpeg.deadline == VideoMuxerFFmpeg.Deadline_GoodQuality?
                                1:
                          VideoMuxerFFmpeg.deadline == VideoMuxerFFmpeg.Deadline_BestQuality?
                                2:
            ...
            model: ListModel {
                ListElement {
                    text: qsTr("Realtime")
                    deadline: VideoMuxerFFmpeg.Deadline_Realtime
                }
                ListElement {
                    text: qsTr("Good quality")
                    deadline: VideoMuxerFFmpeg.Deadline_GoodQuality
                }
                ListElement {
                    text: qsTr("Best quality")
                    deadline: VideoMuxerFFmpeg.Deadline_BestQuality
                }
                ListElement {
                    text: qsTr("Custom")
                    deadline: VideoMuxerFFmpeg.Deadline_Realtime
            ...
            onCurrentIndexChanged: VideoMuxerFFmpeg.deadline = model.get(currentIndex).deadline
        }
        TextField {
            text: VideoMuxerFFmpeg.deadline
            ...
            onTextChanged: VideoMuxerFFmpeg.deadline = Number(text)
        }
    }
}
