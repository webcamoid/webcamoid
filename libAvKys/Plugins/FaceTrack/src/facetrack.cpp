...
{
    public:
        FaceTrack *self {nullptr};
        QString m_description {QObject::tr("Face tracking")};
        ...
        QRect m_lastBounds;
        IAkVideoFilterPtr m_faceDetectFilter {akPluginManager->create<IAkVideoFilter>("VideoFilter/FaceDetect")};
        QMutex m_mutex;

        explicit FaceTrackPrivate(FaceTrack *self);
...
FaceTrack::FaceTrack(QObject *parent):
      QObject(parent)
{
    this->d = new FaceTrackPrivate(this);
...
QString FaceTrack::description() const
{
    return this->d->m_description;
}

AkElementType FaceTrack::type() const
{
    return this->d->m_type;
}

AkElementCategory FaceTrack::category() const
{
    return this->d->m_category;
}

void *FaceTrack::queryInterface(const QString &interfaceId)
{
...
IAkElement *FaceTrack::create(const QString &id)
{
    Q_UNUSED(id)
...
int FaceTrack::registerElements(const QStringList &args)
{
    QString pluginPath;
    ...
    AkPluginInfo pluginInfo("VideoFilter/FaceTrack",
...
QString FaceTrack::haarFile() const
{
    return this->d->m_haarFile;
}

QSize FaceTrack::scanSize() const
{
    return this->d->m_scanSize;
}

int FaceTrack::faceBucketSize() const
{
    return this->d->m_faceBucketSize;
}

int FaceTrack::faceBucketCount() const
{
    return this->d->m_faceBuckets.length();
}

int FaceTrack::expandRate() const
{
    return this->d->m_expandRate;
}

int FaceTrack::contractRate() const
{
    return this->d->m_contractRate;
}

QRect FaceTrack::facePadding() const
{
    return this->d->m_facePadding;
}

QRect FaceTrack::faceMargin() const
{
    return this->d->m_faceMargin;
}

AkFrac FaceTrack::aspectRatio() const
{
    return this->d->m_aspectRatio;
}

bool FaceTrack::overrideAspectRatio() const
{
    return this->d->m_overrideAspectRatio;
}

bool FaceTrack::lockedViewport() const
{
    return this->d->m_lockedViewport;
}

bool FaceTrack::debugModeEnabled() const
{
    return this->d->m_debugModeEnabled;
}

void FaceTrack::deleteThis(void *userData) const
{
    delete reinterpret_cast<FaceTrack *>(userData);
}

QString FaceTrack::controlInterfaceProvide(const QString &controlId) const
{
    Q_UNUSED(controlId)
...
void FaceTrack::controlInterfaceConfigure(QQmlContext *context,
...
AkPacket FaceTrack::iVideoStream(const AkVideoPacket &packet)
{
    ...
    if (this->d->m_lockedViewport) {
        ...
        this->d->m_faceDetectFilter->call("detectFaces",
                                          &results,
                                          QVariantList {packet});
        auto detectedFaces = results.value(0).value<QVector<QRect>>();
...
void FaceTrack::setHaarFile(const QString &haarFile)
{
    if (this->d->m_haarFile == haarFile)
        return;

    this->d->m_faceDetectFilter->property<IAkPropertyString>("haarFile")->setValue(haarFile);
    this->d->m_haarFile = haarFile;
    emit this->haarFileChanged(haarFile);
}

void FaceTrack::setScanSize(const QSize &scanSize)
{
    if (this->d->m_scanSize == scanSize)
        return;

    this->d->m_faceDetectFilter->property<IAkPropertySize>("scanSize")->setValue(scanSize);
    this->d->m_scanSize = scanSize;
    emit this->scanSizeChanged(scanSize);
}

void FaceTrack::setFaceBucketSize(int seconds)
{
    if (this->d->m_faceBucketSize == seconds)
        return;

    this->d->m_faceBucketSize = seconds;
    emit this->faceBucketSizeChanged(seconds);
}

void FaceTrack::setFaceBucketCount(int count)
{
...
void FaceTrack::setExpandRate(int rate)
{
    if (this->d->m_expandRate == abs(rate))
        return;

    this->d->m_expandRate = abs(rate);
    emit this->expandRateChanged(abs(rate));
}

void FaceTrack::setContractRate(int rate)
{
    if (this->d->m_contractRate == abs(rate))
        return;
...
void FaceTrack::setFacePadding(const QRect &facePadding)
{
...
void FaceTrack::setFaceMargin(const QRect &faceMargin)
{
...
void FaceTrack::setAspectRatio(const AkFrac &aspectRatio)
{
...
void FaceTrack::setOverrideAspectRatio(bool overrideAspectRatio)
{
...
void FaceTrack::setLockedViewport(bool lockViewport)
{
...
void FaceTrack::setDebugModeEnabled(bool enabled)
{
...
void FaceTrack::resetHaarFile()
{
    this->setHaarFile(":/FaceDetect/share/haarcascades/haarcascade_frontalface_alt.xml");
}

void FaceTrack::resetScanSize()
{
    this->setScanSize({160, 120});
}

void FaceTrack::resetFaceBucketSize()
{
    this->setFaceBucketSize(1);
}

void FaceTrack::resetFaceBucketCount()
{
    this->setFaceBucketCount(5);
}

void FaceTrack::resetExpandRate()
{
    this->setExpandRate(30);
}

void FaceTrack::resetContractRate()
{
    this->setContractRate(5);
}

void FaceTrack::resetFacePadding()
{
    this->setFacePadding({20, 50, 0, 80});
}

void FaceTrack::resetFaceMargin()
{
    this->setFaceMargin({30, 30, 0, 0});
}

void FaceTrack::resetAspectRatio()
{
    this->setAspectRatio({16, 9});
}

void FaceTrack::resetOverrideAspectRatio()
{
    this->setOverrideAspectRatio(false);
}

void FaceTrack::resetLockedViewport()
{
    this->setLockedViewport(false);
}

void FaceTrack::resetDebugModeEnabled()
{
    this->setDebugModeEnabled(false);
}

FaceTrackPrivate::FaceTrackPrivate(FaceTrack *self):
    self(self)
{

}

QRect FaceTrackPrivate::calculateNewBounds(const QRect &targetBounds,
                                           const QSize &maxCropSize,
                                           const QSize &srcSize)
{
...
void FaceTrackPrivate::collectFaces(const QVector<QRect> &vecFaces)
{
    /*
