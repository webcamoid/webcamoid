...
namespace AkVCam
{
    ...
    class CStreamRead
    {
        public:
            ...
            CStreamRead(const unsigned char *stream,
                        ...
                        bool isBigEndian=false,
                        Mode mode=ModeRead);
            CStreamRead(const char *stream, size_t size, Mode mode);
            CStreamRead(const unsigned char *stream, size_t size, Mode mode);
            CStreamRead(const char *stream, bool isBigEndian);
            CStreamRead(const unsigned char *stream, bool isBigEndian);
    ...
    template<>
    inline std::string CStreamRead::read<std::string>()
    {
        if (!this->data())
            return {};

        std::string str(this->data());
        this->seek(int(str.size() + 1));
